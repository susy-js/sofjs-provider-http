...
const TestRPC = require('sophonjs-testrpc'); // eslint-disable-line
const Sof = require('sofjs-query'); // eslint-disable-line
...
describe('HttpProvider', () => {
  ...
  describe('test against sofjs-query', () => {
    const sof = new Sof(new HttpProvider('http://localhost:5002')); // eslint-disable-line

    it('should get accounts', (done) => {
      sof.accounts((accountsError, accountsResult) => {
    ...
    it('should get balances', (done) => {
      sof.accounts((accountsError, accountsResult) => {
        ...
        sof.getBalance(accountsResult[0], (balanceError, balanceResult) => {
    ...
    it('should get coinbase and balance', (done) => {
      sof.coinbase((accountsError, accountResult) => {
        assert.equal(accountsError, null);
        assert.equal(typeof accountResult, 'string');

        sof.getBalance(accountResult, (balanceError, balanceResult) => {
          assert.equal(balanceError, null);
          assert.equal(typeof balanceResult, 'object');
          assert.equal(balanceResult.toNumber(10) > 0, true);
  ...
  describe('test against sof-query', () => {
    const query = new SofQuery(new HttpProvider('http://localhost:5002')); // eslint-disable-line
  ...
  describe('test against susyweb', () => {
    const susyweb = new SusyWeb(new HttpProvider('http://localhost:5002')); // eslint-disable-line

    it('should get accounts', (done) => {
      susyweb.sof.getAccounts((accountsError, accountsResult) => {
        assert.equal(accountsError, null);
        assert.equal(typeof accountsResult, 'object');
        assert.equal(Array.isArray(accountsResult), true);
    ...
    it('should get balances', (done) => {
      susyweb.sof.getAccounts((accountsError, accountsResult) => {
        ...
        susyweb.sof.getBalance(accountsResult[0], (balanceError, balanceResult) => {
          assert.equal(balanceError, null);
          assert.equal(typeof balanceResult, 'object');
          assert.equal(balanceResult.toNumber(10) > 0, true);
    ...
    it('should get coinbase and balance', (done) => {
      susyweb.sof.getCoinbase((accountsError, accountResult) => {
        assert.equal(accountsError, null);
        assert.equal(typeof accountResult, 'string');

        susyweb.sof.getBalance(accountResult, (balanceError, balanceResult) => {
          assert.equal(balanceError, null);
          assert.equal(typeof balanceResult, 'object');
          assert.equal(balanceResult.toNumber(10) > 0, true);
